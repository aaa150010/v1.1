...
const importAWSCSS =
  AWSDevParams.importAWSCSS == null ? [] : AWSDevParams.importAWSCSS;
const importAWSJS =
  AWSDevParams.importAWSJS == null ? [] : AWSDevParams.importAWSJS;
const AWSJSAndCSSImportArray = [] as any[];
for (const k of importAWSCSS) {
  AWSJSAndCSSImportArray.push(
...
for (const k of importAWSJS) {
  AWSJSAndCSSImportArray.push(
    "<script type='text/javascript' src='" + AWSJSAndCSSPath + k + "'></script>"
  );
}
...
const replaceHtml = (html: string, htmlMap: any) => {
  for (const key in htmlMap) {
    if (key == "publicDir") {
      ...
      html = html.replace(
        new RegExp("\\$\\{" + key + "\\}", "g"),
        typeof htmlMap[key] == "object"
          ? JSON.stringify(htmlMap[key])
          : htmlMap[key]
      );
    }
  }
